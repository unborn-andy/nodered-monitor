<template>
  <div>
    <b-container fluid class="pr-5">
      <h2 class="ml-4 mt-4">Energy Total Chart ( kWh )</h2>
      <b-row align-v="end" class="ml-4 mt-4">
        <!-- <b-col cols="3">
          <label for="datepicker1">Select Year :</label>
          <date-picker v-model="startDate" format="YYYY" valueType="format" type="year" :append-to-body="false" :popup-style="{ left: 0, top: '100%', marginTop: '10px' }"></date-picker>
        </b-col> -->
        <b-col cols="4">
        ...
        <b-col cols="7" class="d-flex align-items-end justify-content-end">
      ...
      <b-row class="justify-content-start p-2 ml-4 mt-4">
        <b-col cols="12">
...
export default {
  ...
  data() {
    return {
      ...
      startDate: moment().format("YYYY"), // "2020-02-02 02:29:30"
          ...
          xAxes: [
            {
              ...
              time: {
                parser: "YYYY",
                // unit: 'day',
                displayFormats: { year: "YYYY", month: "YYYY", day: "YYYY", hour: "YYYY" },
  ...
  mounted() {
    ...
    uibuilder.onChange("msg", (json) => {
      ...
      if (json.cmd == "chartEnergyTotal") {
      console.info("Msg received from Node-RED server in EnergyTotal:", json);
  ...
  methods: {
    ...
    downloadCSV() {
      // let startDate = moment(this.startDate, 'YYYY').format('YYYY-01-01 00:00:00')
      //let endDate = moment(this.startDate, 'YYYY').format('YYYY-12-31 24:00:00')
      let cmd = "csvEnergyTotal";

      //let url = `http://${window.location.host}/csv?startDate=${startDate}&endDate=${endDate}&cmd=${cmd}=&table=${this.table}`;
      let url = `http://${window.location.host}/csv?cmd=${cmd}&table=${this.table}`;
      console.log(url);
    ...
    chartToggleAll() {
      ...
      let chartId = this.$children[0]._data._chart.id;
      this.$children[0]._data._chart.data.datasets.forEach((d) => {
        d._meta[chartId].hidden = !d._meta[chartId].hidden;
      });
      //this.$children[1]._data._chart.data.datasets.forEach(d =>  d.hidden = !d.hidden)
      this.$children[0]._data._chart.update();
    },
  },
};
</script>
