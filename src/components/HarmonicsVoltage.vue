<template>
  <div>
    <b-container fluid class="pr-5">
      <h2 class="ml-4 mt-4">Harmonics Voltage Daily Chart</h2>
...
export default {
  ...
  mounted() {
    uibuilder.onChange("msg", (json) => {
      if (json.cmd == "chartHarmonicsVoltage") {
        console.info("Msg received from Node-RED server in Harmonics Voltage:", json);
  ...
  methods: {
    requestData() {
      ...
      console.log("Message sent from Harmonics Voltage:", this.msgSent);
    },
